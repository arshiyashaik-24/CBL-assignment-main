...
class GamePanel extends JFrame {
    ...
    GamePanel() {
        setTitle("⚡ Connect the Wires — Cyber Grid");
        ...
        livesLabel.setBounds(15, 10, 200, 30);
        ...
        // Transparent overlay for drawing wires
        wireCanvas = new WireCanvas();
        wireCanvas.setOpaque(false);
        wireCanvas.setBounds(0, 0, 520, 420);
        panel.add(wireCanvas);
    ...
    private void selectRight(int index) {
        ...
        // Calculate centers (relative to wireCanvas)
        Point leftCenter = SwingUtilities.convertPoint(left.getParent(),
                left.getX() + left.getWidth(), left.getY() + left.getHeight() / 2, wireCanvas);
        Point rightCenter = SwingUtilities.convertPoint(right.getParent(),
                right.getX(), right.getY() + right.getHeight() / 2, wireCanvas);
    ...
    class WireCanvas extends JPanel {
        @Override
